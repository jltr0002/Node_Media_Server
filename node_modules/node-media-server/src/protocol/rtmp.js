...
const streamCatalog = require('../utils/streamCatalog.js');
...
class Rtmp {
  ...
  onPublish = (invokeMessage) => {
    ...
    // Assuming tcUrl is available from the connect command object
    const streamUrl = `${this.connectCmdObj.tcUrl}/${this.streamName}`;
    console.log('streamRUL value:', streamUrl);

    streamCatalog.addStream({
      name: this.streamName,
      url : streamUrl
    });
    
    // Route the stream to the appropriate service
    services.handleStream({
      name: this.streamName,
      url: streamUrl
    });
  ...
  onDeleteStream = (invokeMessage) => {
    console.log('Stream deleted, checking for ffmpeg process...');
    // Assume that the stream name was stored when publishing.
    const streamName = this.streamName; // Ensure this is set appropriately.
    
    processRegistry.stopProcess(streamName);
  };
