...
function buildFfmpegArgs(inputUrl, outputDir) {
  return [
    ...
    '-filter_complex',
      ...
      "[vid2]scale=-2:720[vid2out]",
    '-map', '0:a?', '-c:a', 'aac', '-b:a', '128k',
    '-map', '[vid1]', '-c:v:0', 'libx264', '-b:v:0', '4500k',
    '-map', '[vid2out]', '-c:v:1', 'libx264', '-b:v:1', '2500k',
    '-preset', 'veryfast',
...
function packageWithShaka(outputDir, streamName) {
  const packagerCmd = `
    packager ^
      input=${path.join(outputDir, 'stream_0_000.mp4')},stream=video,init_segment=${path.join(outputDir, 'init-144p.mp4')},segment_template=${path.join(outputDir, 'seg-144p-$Number$.m4s')} ^
      input=${path.join(outputDir, 'stream_1_000.mp4')},stream=video,init_segment=${path.join(outputDir, 'init-720p.mp4')},segment_template=${path.join(outputDir, 'seg-720p-$Number$.m4s')} ^
      --profile live ^
